...
public class ConnectActivity extends BaseActivity {
    ...
    EditText etPort;
    @BindView(R.id.tv_connect_reader)
    Button tvConnectReader;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        spUtil = MApplication.getInstance().getSpUtil();
        initTcp();
        if (!spUtil.getIp().equals("")) {
            etAddress.setText(spUtil.getIp());
        }
        if (spUtil.getPort() != 0) {
            etPort.setText(String.valueOf(spUtil.getPort()));
        }
    }
