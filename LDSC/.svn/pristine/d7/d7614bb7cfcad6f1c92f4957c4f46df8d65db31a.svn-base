	...
	public static final DefaultHttpClient getHttpClient()
	{
		if (mHttpClient == null)
		{
			...
			ConnManagerParams.setMaxConnectionsPerRoute(params, new ConnPerRouteBean(
					BasicConfig.HttpMaxConnectionsPerRoute));
			// 链接等待超时
			...
			// mHttpClient.setRedirectHandler(new MyRedirectHandler());
			// 编码
			// HttpProtocolParams.setContentCharset(mHttpClient.getParams(), HTTP.DEFAULT_CONTENT_CHARSET);
			// Expect 100 Continue
			// HttpProtocolParams.setUseExpectContinue(mHttpClient.getParams(), true);
		}
		return mHttpClient;
	}
