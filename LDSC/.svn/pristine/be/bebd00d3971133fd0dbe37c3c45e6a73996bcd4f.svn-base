...
public class StockTaskDetailsActivity extends BaseActivity implements IDataFragmentView<String> {
    ...
    public void init() {
        //唯一码
        List<TakeStockEpcCollect> epcList =
                epcDBOperator.getItemsByParameters(null, TakeStockEpcCollectDao.Properties.TaskId.eq(mCheckTask.getId()));
        //条码
        List<TakeStockSkuCollect> skuList =
                skuDBOperator.getItemsByParameters(null, TakeStockSkuCollectDao.Properties.TaskId.eq(mCheckTask.getId()));
        epcNum = epcList.size();
    ...
    private void initView() {
        ...
        List<TakeStockOrder> takeStockOrders =
                orderDBOperator.getItemsByParameters(null, TakeStockOrderDao.Properties.Id.eq(mCheckTask.getOrderId()));
