    ...
    public void ShowData(String string) {
        if (string.equals("S")) {
            ...
            List<TakeStockEpcCollect> epcList =
                    mActivity.epcDBOperator.getItemsByParameters(null, TakeStockEpcCollectDao.Properties.TaskId.eq(mActivity.mCheckTask.getId()));
            //条码
            List<TakeStockSkuCollect> skuList =
                    mActivity.skuDBOperator.getItemsByParameters(null, TakeStockSkuCollectDao.Properties.TaskId.eq(mActivity.mCheckTask.getName()));
